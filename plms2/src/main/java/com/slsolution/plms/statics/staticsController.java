...
//			int result = Database.getInstance().update("Json.insertStatisticsDeadlineProcess", rst);
//
//			if (result > 0) {
	...
	// 토지 관리 현황(권리확보현황) 조회리스트 리턴
		public HashMap selectTogiMgtStateListReturn(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			try {
				...
				//gukyujiList = (ArrayList) Database.getInstance().queryForList("Json.selectTogiMgtGoverSearch", params);
//				issueList = (ArrayList) Database.getInstance().queryForList("Json.searchStatisticsTojiCount", params);
