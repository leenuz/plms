...
const privateUseSelectEvet = (selectBtn, selectList , selectListText , selectHiddenBox) => {
       ...
       if( selectWrappers.length > 0){
        for(let d = 0; d <= selectWrappers.length - 1; d++){
           ...
           if(surfaceSelectListMember[d].classList.contains("otherDefault01")){ 
              surfaceSelectsTitleBtn[d].innerText = `지사선택필요`;
           }
          else if(surfaceSelectListMember[d].classList.contains("otherDefault02")){
             surfaceSelectsTitleBtn[d].innerText = `허가관청선택필요`;
           }
          else if(surfaceSelectListMember[d].classList.contains("selectChoice")){
            surfaceSelectsTitleBtn[d].innerText = `선택`;
           }
