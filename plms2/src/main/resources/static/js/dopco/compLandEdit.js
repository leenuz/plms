...
const createCustomLiComLandEdit = () => {
...
const compLandEditSelectboxClickEvent01 = () => {
    const contentContainer = document.querySelectorAll('#compLandEdit .contWrap');
    const customSelectView = document.querySelectorAll('.customSelectView');
...
compLandEditSelectboxClickEvent01();
...
const compLandEditSelectEvent02 = () => {

    const compLandEditContainer = document.querySelectorAll('#compLandEdit .contWrap');

    compLandEditContainer.forEach((wrap) => {
        wrap.addEventListener('click', function (event) {
...
compLandEditSelectEvent02();

// 후순위 권리 click 이벤트

const compLandEditInfoAddBtnEvent = () => {
    const infoContentsDetailBox = document.querySelector('#compLandEdit .subordinateRts .contWrap');
        ...
        if (event.target.classList.contains('addBtn')) {
            ...
            for (let i = 1; i <= 8; i++) {
                ...
                if (i == 1 ) {
                    ...
                    checkboxInput.id = `compLandEdit_checkbox_${Date.now()}_${i}`;
...
compLandEditInfoAddBtnEvent();


// check박스 체크되었을 때

const checkboxEvent = () => {
    const checkboxSection = document.querySelector('#compLandEdit .subordinateRts');
...
const compLandEditFileEvent = () => {
    const compLandEdit_myPcFiles = document.getElementById('compLandEdit_myPcFiles');
    // input[type file]을 가진 제일 큰 부모
    const attachFileInfo = compLandEdit_myPcFiles.closest('.attachFileInfo');
    // 업로드시 보이는 라벨
    ...
    compLandEdit_myPcFiles.addEventListener('change', function () {


        console.log(compLandEdit_myPcFiles.files)
        // 기존의 ul 초기화
        ...
        if (compLandEdit_myPcFiles.files.length > 0) {

            for (let i = 0; i <= compLandEdit_myPcFiles.files.length - 1; i++) {
                const thisFileName = compLandEdit_myPcFiles.files[i].name;
                const thisFileSize = compLandEdit_myPcFiles.files[i].size;
                const thisFileType = compLandEdit_myPcFiles.files[i].type;
            ...
            // 값 잘 담겼는지 확인
            console.log(compLandEdit_myPcFiles.value)
    ...
    fileUploadAfter.addEventListener('click', function (event) {
        if (event.target.classList.contains('fileDeleteBtn')) {
            ...
            console.log(compLandEdit_myPcFiles.files.length);
    ...
    const removeFile = (fileNameToRemove) => {
        const filesArray = Array.from(compLandEdit_myPcFiles.files);
        ...
        compLandEdit_myPcFiles.files = newDataTransfer.files; // 새로운 files 설정
    };

}

compLandEditFileEvent();

/* js추가 검색팝업오픈 */

const compLandEditSearchOpenPopUp = () => {

    const compLandEditsSearchBtn = document.querySelector(".compLandEditsSearchBtn");
    const compLandEditSearchResultPop = document.querySelector(".compLandEditSearchResultPopWrapper");
    let htmlFilePath = '/components/popuphtml/searchResultsPopup.html'; // 삽입할 html 파일 경로

    if (compLandEditsSearchBtn) {

        let xhr = new XMLHttpRequest();
        xhr.open('GET', htmlFilePath, true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                compLandEditSearchResultPop.innerHTML = xhr.responseText;
                runScriptsInElement(compLandEditSearchResultPop); // 삽입된 html내 스크립트 실행 함수 호출
            }
        };
        xhr.send();
        console.log('compLandEditSearchResultPop 작동');
        compLandEditsSearchBtn.addEventListener("click", () => {

            const popupOpen = document.querySelector("#searchResultsPopup .popupWrap");
            compLandEditsSearchBtn.classList.add("open");
