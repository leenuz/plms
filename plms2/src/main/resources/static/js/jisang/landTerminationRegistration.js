...
$(document).on("click", "#save_btn", function() {
  ...
  if (!formData.req_doc_file02) {
      errors.push('필수 첨부파일 2번이 누락되었습니다.');
  }
  if (!formData.req_doc_file03) {
      errors.push('필수 첨부파일 3번이 누락되었습니다.');
  }
  if (!formData.req_doc_file04) {
      errors.push('필수 첨부파일 4번이 누락되었습니다.');
  }
	if (!formData.req_doc_file05) {
      errors.push('필수 첨부파일 5번이 누락되었습니다.');
  }
  if (!formData.req_doc_file06) {
      errors.push('필수 첨부파일 6번이 누락되었습니다.');
  }
  if (!formData.req_doc_file07) {
      errors.push('필수 첨부파일 7번이 누락되었습니다.');
  }
...
function landTerminationSave(params){
   // loadingShow();
   var url= "/land/jisang/landTerminationSave";
   	$.ajax({
