...
// 토지개발대상 상세보기 버튼
$(document).on('click', '.contBoxMini02 .viewDetailButton', function() {
    var pnu = $(this).data('pnu');  // data-pnu 속성 값 가져오기
    console.log("pnu: ", pnu);

    // pnu에 해당하는 dm_dosi_no를 확인하기 위한 요청
    $.ajax({
        url: "/togi/checkPnu",  // pnu 확인을 위한 새로운 API 경로
        type: "GET",
        data: { pnu: pnu },  // pnu를 파라미터로 넘김
        success: function(response) {
            // 성공적으로 dm_dosi_no를 가져왔으면, landDevInfo로 이동
            var dmDosiNo = response.dm_dosi_no;
            console.log("dm_dosi_no: ", dmDosiNo);
            window.location.href = "/togi/landDevInfo?idx=" + dmDosiNo;
        },
        error: function(xhr) {
            // pnu가 없으면 alert 띄우기
            if (xhr.status === 404) {
                alert("토지개발대상토지가 아닙니다.");
            } else {
                alert("오류가 발생했습니다.");
            }
        }
