...
const surfaceInquireSelectEvet = (selectBtn, selectList , selectListText , selectHiddenBox, checkBoxName) => {
  ...
  console.log("###########surfaceInquireSelectEvet#################");
console.log(selectWrappers);
    ...
    const initialValues = () => {
        if( selectWrappers.length > 0){
         for(let d = 0; d <= selectWrappers.length - 1; d++){
            ...
            if(surfaceSelectListMember[d].classList.contains("checkBoxsContent")){ 
               surfaceSelectsTitleBtn[d].innerText = `항목을 선택해주세요`;
            }
            if(surfaceSelectListMember[d].classList.contains("selectChoice")){
    ...
    for(let i = 0; i <= selectWrappers.length - 1; i++){
        ...
        for(let x = 0; x <= selectValues.length - 1; x++){
			console.log(selectValues[x].text);
       ...
       surfaceSelectList.forEach((list) => {
          list.addEventListener("click" , (event) => {
            ...
            if (event.target.nodeName === "P") {
                ...
                //지목셀렉박스 클릭시
                if( currentSelect === "surfaceInquireSelectBox01_4"){
