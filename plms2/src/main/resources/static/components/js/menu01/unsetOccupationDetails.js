...
const unsetOccupationDetailsFileEvent = () => {
    const unsetOccupationDetails_myPcFiles = document.getElementById('unsetOccupationDetails_myPcFiles');
    const parcelAttachments = unsetOccupationDetails_myPcFiles.closest('.parcelAttachments');
    const fileContWrap = parcelAttachments.querySelector('.contWrap');
    const fileTitles = fileContWrap.querySelector('#unsetOccupationDetails .parcelAttachments .depth1 .titles');
    const contentsBox = fileContWrap.querySelector('.contentsBox');
    var fileInfoName = '';

    unsetOccupationDetails_myPcFiles.addEventListener('change', function () {
        ...
        if (unsetOccupationDetails_myPcFiles.files.length > 0) {
            for (let i = 0; i <= unsetOccupationDetails_myPcFiles.files.length - 1; i++) {

                const thisFileName = unsetOccupationDetails_myPcFiles.files[i].name;

                fileInfoName = thisFileName;

                const checkboxLi = `<li class="content01 content"><input type="checkbox" id="unsetOccupationcustomCheckbox${i}"><label for="unsetOccupationcustomCheckbox${i}"></label>
                                    </li>`;
            ...
            // 값 잘 담겼는지 확인
            console.log(unsetOccupationDetails_myPcFiles.value)

            // titles border주기
            fileTitles.classList.add('active');

            if (unsetOccupationDetails_myPcFiles.files.length >= 5) {
...
unsetOccupationDetailsFileEvent();

// occupationDetails memosection 추가, 삭제, 등록 이벤트

const unsetOccupationMemoAddBtnEvent = () => {
    const memoContentsDetailBox = document.querySelector('#unsetOccupationDetails .memoSection .contentDetailBox');
...
unsetOccupationMemoAddBtnEvent();
