const usePermitRegisterInfoAddBtnEvent = () => {
    const infoContentsDetailBox = document.querySelector('#usePermitRegister .targetLand .contWrap');
    ...
    infoContentsBox.addEventListener('click', function (event) {
        ...
        if (event.target.classList.contains('addBtn')) {
            ...
            for (let i = 1; i <= 5; i++) {
                ...
                if (i == 1) {
                    ...
                    addressBtn.classList.add('landPopBtn');
        ...
        // 토지 추가 검색 팝업 열기
        if (event.target.classList.contains('landPopBtn')) {
...
usePermitRegisterInfoAddBtnEvent();
