...
const landRightsRegistrationInfoAddBtnEvent = () => {
    const infoContentsContWrap = document.querySelector('#landRightsRegistration .ownerInfo .contWrap');
    const infoContentsBox = infoContentsContWrap.querySelector('.contentsBox');
    const infoAddBtn = infoContentsContWrap.querySelectorAll('.addBtn');
    const infoContents = infoContentsContWrap.querySelectorAll('.contents');
    const infoTitles = infoContentsContWrap.querySelector('.titles');
            ...
            for (let i = 1; i <= 6; i++) {
                ...
                if (i == 6) {
                    ...
                    for (let d = 0; d < 2; d++) {
                        ...
                        infoDiv.appendChild(infoMiniBtn);

                        infoLi.appendChild(infoDiv);
                    }
...
landRightsRegistrationInfoAddBtnEvent();
...
const landRightsRegistrationFileEvent = () => {
    const landRightsRegistration_myPcFiles = document.getElementById('landRightsRegistration_myPcFiles');
    // input[type file]을 가진 제일 큰 부모
    const attachFileInfo = landRightsRegistration_myPcFiles.closest('.attachFileInfo');
    // 업로드시 보이는 라벨
    ...
    landRightsRegistration_myPcFiles.addEventListener('change', function () {
        ...
        if (landRightsRegistration_myPcFiles.files.length > 0) {

            for (let i = 0; i <= landRightsRegistration_myPcFiles.files.length - 1; i++) {
                const thisFileName = landRightsRegistration_myPcFiles.files[i].name;
                const thisFileSize = landRightsRegistration_myPcFiles.files[i].size;
                const thisFileType = landRightsRegistration_myPcFiles.files[i].type;
            ...
            // 값 잘 담겼는지 확인
            console.log(landRightsRegistration_myPcFiles.value)
            ...
            landRightsRegistration_myPcFiles.value = '';
            defaultFileUploadWrap.forEach((wrap) => {
                wrap.classList.remove('active');
            })
            defaultFileUploadWrap[0].classList.add('active');
    ...
    fileUploadAfter.addEventListener('click', function (event) {
        if (event.target.classList.contains('fileDeleteBtn')) {
            ...
            console.log(landRightsRegistration_myPcFiles.files.length);
    ...
    const removeFile = (fileNameToRemove) => {
        const filesArray = Array.from(landRightsRegistration_myPcFiles.files);
        ...
        landRightsRegistration_myPcFiles.files = newDataTransfer.files; // 새로운 files 설정
    };

}

landRightsRegistrationFileEvent();
